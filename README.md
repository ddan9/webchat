...
- уведомление о входе человека в чат? (было раньше такое). Решение: повесить все в update.php сделать опцию на функцию, кулдаун по времени последнего сообщения, выводить нотифи. По умолчанию аноним если не включена опция запоминания, иначе ник
- бан пользователя по адресу (либо белый список)
- переделать весь пост под js?
- все таки надо ввести ники на файлы
...
- можно типо когда выпадает ошибка из-за удаления сообщения из базы (последовательность ломается) незаметно обновить фрейм
