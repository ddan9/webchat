...
- доктайпы?
- убрать бэкграунд колоры и тестить на винде с темной темой
- при изменении картинки на кнопке, они не прогружаются
- залить ещё пару демок
...
- поменять все кавычки (двойные и одинарные) местами и убрать отовсюду фигурные скобки
- В СИСТЕМЕ АУТЕНТИФИКАЦИИ ПЕРЕМЕННАЯ $protocol ХЕР ПОЙМИ ОТКУДА ВЗЯТА
- взять из модуля логаута часть с протоколом и переделать под HTTP, разделить переменную protocol на HTTP и URI и скинуть всё это дело в пресеты (это всё к верхней задаче)
- поиск в клиента
